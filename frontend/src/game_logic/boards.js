...
// dificulties with
const generateGameBoards = questions => {
  // questions input is entire pojo of
  ...
  let round3QuestionsKeys = categories.splice(0, 5); // remainder of keys for round 3
  ...
  Object.keys(columns).forEach(category => {
    ...
    for (let i = 0; i < 5; i++) {
      // easy for 0, 1 & 2 for medium, 3 & 4 for hard
      if (i === 0) {
        //easy question
        let randIndex = Math.floor(
          Math.random() * columns[category]["easy"].length
        );
        let randQuestion = columns[category]["easy"][randIndex];
        round1Questions[category].push(randQuestion);
      } else if (i === 1 || i === 2) {
        let randIndex = Math.floor(
          Math.random() * columns[category]["medium"].length
        );
        ...
        let randIndex = Math.floor(
          Math.random() * columns[category]["hard"].length
        );
